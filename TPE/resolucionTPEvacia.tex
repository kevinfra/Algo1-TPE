...
\begin{problema}{nuevoN}{an : \ent, al : \ent, s : \ent, spaw : [(Vampiro, \ent, \ent)]}{Nivel}
	...
	\asegura[elGamerNoMuereSpawnea]{(\forall s \in spawning(res))(((\forall i \in [0..|spawning(res)|-1))trd(spawning(res)_i)\leq trd(spawning(res)_{i+1}))\land \newline ((\forall j \in [0..|spawning(n)|-1), trd(spawning(res)_j)==trd(spawning(res)_{j+1}) sgd(spawning(res)_j)\leq sgd(spawning(res)_{j+1}))}
\end{problema}
...
\begin{problema}{pasarTurno}{n: Nivel}{}
	...
	\asegura{(\forall f \in flores(pre(n)), Explotar \not\in Habilidades(prm(f)) \land trd(f) - danoVampiros(pre(n), sgd(f)) \leq 0) \newline f \not\in flores(n)}
	\asegura{(\forall f \in flores(pre(n)), Explotar \not\in Habilidades(prm(f)) \land trd(f)) - danoVampiros(pre(n), sgd(f)) \geq 0) \newline (\exists f1 \in flores(n)) \ prm(f1)==prm(f) \land sgd(f1)==sgd(f) \land trd(f1)==trd(f)-danoVampiros(pre(n), sgd(f))}
	...
	\asegura{(\forall v \in vampiros(pre(n)), trd(v) - danoFlores(pre(n), sgd(v)) \leq 0) \newline v \not\in vampiros(n)}
	\asegura{(\forall v \in vampiros(pre(n)), trd(f)) - danoFlores(pre(n), sgd(v)) > 0) \newline (\exists v1 \in vampiros(n)) \ prm(v1)==prm(v) \land seMueve(v1,v,n,pre(n)) \land trd(v1)==trd(v)-danoVampiros(pre(n), sgd(f))}
...
\begin{problema}{altoCheat}{j: Juego, i: \ent}{}
	...
	\asegura{mismos(flores(j),flores(pre(j))}
	\asegura{mismos(vampiros(j),vampiros(pre(j))}
	\asegura[losAnterioresSonIguales3] {(\forall k \leftarrow (0..i)) nivelesIguales(niveles(j)_{k},niveles(pre(j))_{k})}
	\asegura[esElNivelQueQuiero3] {ancho(niveles(pre(j))_i)==ancho(niveles(j)_i)\land alto(niveles(pre(j))_i)==alto(niveles(j)_i) \land \newline turno(niveles(pre(j))_i)==turno(niveles(j)_i) \land soles(niveles(pre(j))_i)==soles(niveles(j)_i) \land mismos(flores(niveles(pre(j))_i),flores(niveles(j)_i)) \land \newline spawning(niveles(pre(j))_i)==spawning(niveles(j)_i)  }
	\asegura[losSiguientesSonIguales3] {(\forall k \leftarrow (i..|niveles(j)|)) \newline nivelesIguales(niveles(j)_{k},niveles(pre(j))_{k})}
\end{problema}

\begin{problema}{muyDeExactas}{j: Juego}{\bool}
	\asegura[sumaDeDos]{ifThenElse((|nivGsL(j)| \geq 3 \ \land \ nivGsL(j)_0 == 1 \ \land \ nivGsL(j)_1 == 2 \newline \land (\forall k \in [2..|nivGsL(j)|-1]) \ nivGsL(j)_k == nivGsL(j)_{k-1} + nivGsL(j)_{k-2}) \newline \lor (|nivGsL(j)| \geq 2 \land nivGsL(j)_0 == 1 \land nivGsL(j)_1 == 2) \lor \newline (|nivGsL(j)| \geq 1 \land nivGsL(j)_0 == 1), True, False)}
...
\aux{floresOrdenadas}{lF : [Flor]}{[Flor]}{
	((\forall t \in [0..|lF|-1)) \ prm(lF_t) \leq prm(lF_{t+1})) \land ((\forall j \in [0..|lF|-1), prm(lF_j) == prm(lF_{j+1})) sgd(lF_j) \leq sgd(lF_{j+1}))
	}
...
\aux{danoVampiros}{n : Nivel, p : Posicion}{\ent}{
	\sum [cuantoPega(prm(v1)) | v1 \in vampiros(n), sgd(v1)==p]
}
\aux{danoFlores}{n : Nivel, p : Posicion}{\ent}{
...
\aux{delanteExplota}{n : Nivel, p : Posicion}{\bool}{
	(\exists f \in flores(n), Explotar \in habilidades(f)) prm(sgd(f))-1 == prm(p) \land sgd(sgd(f))==sgd(p)}
