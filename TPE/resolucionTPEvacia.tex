...
\begin{problema}{agregarNivelJ}{j: Juego, n: Nivel, i: \ent}{}
	...
	\modifica {j}
	\asegura {(\forall k \leftarrow [0..i)) nivelesIguales(niveles(j)_{k},niveles(pre(j))_{k})}
	\asegura {nivelesIguales(niveles(j)_{i},n)}
	\asegura {(\forall k \leftarrow (i..|niveles(j)|)) nivelesIguales(niveles(j)_{k},niveles(pre(j))_{k-1})}
