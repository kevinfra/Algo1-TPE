...
\begin{problema}{pasarTurno}{n: Nivel}{}
	...
	\asegura{mismos(vampiros(n),vampirosSobrevivientes(vampiros(pre(n)),n,pre(n)) ++ spawnean(n,pre(n)))}
	\asegura[noMeCambiesElNivel]{ancho(n)==ancho(pre(n)) \land alto(n)==alto(pre(n))}
	\asegura[graciasPachamama]{soles(n)==soles(pre(n))+1+ \newline |[1 | j \in flores(pre(n)), Generar \in habilidades(prm(j))]|}
...
\begin{problema}{estaEnJaque}{n:Nivel}{Vampiro}
	...
	\asegura{result \in [prm(v)|v \in vampiros(n),vaAPerderMasVida(v,n)]}
\end{problema}
...
\begin{problema}{nivelesSoleados}{j: Juego}{[Nivel]}
	\asegura[noTerminaron]{mismosNiveles(result,[n|n \in niveles(j),terminado(n)==false])}
	\asegura[estanOrdenados]{(\forall i \in [0..|result|-1)) solesGenerados(result_i) \leq solesGenerados(result_{i+1})}
\end{problema}
...
\aux{da\~{n}oFlores}{n : Nivel, p : Posicion}{\ent}{
	\sum [cuantoPega(prm(f1) | f1 \in flores(n), sgd(sgd(f1)) == sgd(p) \land noHayVampirosEnElMedio(p,f1,n) \land prm(sgd(f1)) \leq prm(p)]
}
...
[(prm(v),seMueve(v,nuevoN),trd(v)-da\~{n}oFlores(viejoN,sgd(v)))|v \in vampiros,trd(v)-da\~{n}oFlores(viejoN,sgd(v)) > 0]
}

\aux{vaAPerderMasVida}{v: (Vampiro,Posicion,Vida), n: Nivel}{\bool}{
(\forall v1 \in vampiros(n)) \newline da\~{n}oFlores(n, sgd(v)) \geq  da\~{n}oFlores(n, sgd(v1))
}
...
[(prm(s),(ancho(n),sgd(s)),vida(prm(s)))|s \in spawning(viejoN), s \not\in spawning(nuevoN)]
}

\aux{solesGenerados}{n: Nivel}{\ent}{soles(n)+1 + |[1 | j \in flores(n), Generar \in habilidades(prm(j))}
\end{document}
